...
class Timeout:
    ...
    async def execute_timeout(self):
        ...
        if self.target_users != self.bot.user:
            # 30% 확률로 5분 타임아웃 적용
            duration = extended_duration if random.random() < 0.3 else default_duration
            ...
            if status == 200:  # HTTP Patch success
                await self.channel.send(f"{users.mention}에게 {duration // 60}분 타임아웃을 적용합니다. 답답하면 네가 뭘 어쩔 수 있는데! 알빠콩!")
                return True
...
async def on_message(message):
    ...
    if message.content.startswith('$duration'):  # 타임아웃 시간 설정
        ...
        if len(message.content) > 9:
            default_duration = int(message.content[9:])
        await channel.send(f"타임아웃 시간: {default_duration // 60}분")
    if author.bot:
        return None
    detection_result = a.word_detect(message.content, 0.8)
    for filter_result in detection_result.values():
        if isinstance(filter_result, dict) and 'result' in filter_result and filter_result['result']:
            ...
            if message not in MSG_TO_TIMEOUT:
